...
I found a space-separated text file, with a tune title (with spaces), a book "code" and a page number:
```
...
...
So, now I have to write `findMatches` and `showMatches`. Each of these could involve pulling in helper libraries (databases, rendering engines), but I started with just the built-in technology.
...
```
There certainly is more elegant code, but this works, and it's simple. And the search was working relatively well.

#### Data Cleansing

Once I saw results, though, I saw errors in the data. I could have looked for a new dataset, but, like lots of things, it seemed like a small problem at first, so I made the small fix. There were typos and OCR errors, as someone had scanned index pages with what is now obsolete technology.
...
### Refection
At no point did i find myself reaching for one of the dozens of tools I've become expert on in the last few years: no need for React or Typescript, CSS frameworks, Postgres. All of the web technologies have grown to help make things easier, even as more and more libraries have added layers on top. Even though Javascript is still Javascript, the type hints of Typescript are available in Javascript code. And CSS has always been powerful and expressive on its own, and the animations possible natively are more than enough for many interactions. And static web hosting has lots going for it, and I could add a database backend and make it work well without having to run any sort of web server. There is no server-side rendering, which is the current trend, but for this app, it works well.

I also found it's much faster to develop on core technologies. The documentation is much easier to find and deal with, as there's less messing with tooling and versions (besides MDN and [caniuse.com](http://caniuse.com/)). To understand this, compare looking up a simple DOM function, versus peeling the skin off CreateReactApp app with the various versions of React and the dozens of dependencies. Plus, moving forward there will be no expiring tools, where I'll be forced into upgrading Webpack, or keep `eslint` rules up-to-date, or upgrade dependencies with security vulnerabilities.

I've made the conscious choice to take the simplest option at every turn. You'd think this would be a recipe for technical debt, but it is just the opposite. It's kept the code small and there's no tool that will go out of fashion that I'll need to migrate away from. There's very little to go wrong. 

Even if it's not useful for your development project, I recommend it as a "cleansing" exercise next time you have a chance.
